...
public class AdherentDAO extends DAO<Adherent>{
    ...
    public ArrayList<Adherent> findAll(int numeroPage, int etat, String recherche) {
        ...
        if(etat==-1)
            updateQuery = "SELECT * FROM ADHERENT a, PERSONNE p WHERE "
    ...
    public int compterLignesRecherche(int etat, String recherche) {
        int nbLignes=0;
        String updateQuery;
        if (etat==-1){
        updateQuery = "SELECT COUNT(*) AS nblignes FROM ADHERENT a, PERSONNE p WHERE "
                + "a.id_Personne=p.id_Personne "
                + "AND(prenom_personne LIKE ? OR nom_personne LIKE ?) ";
